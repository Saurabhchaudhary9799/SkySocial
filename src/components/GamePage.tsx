...
const GamePage: React.FC = () => {
  ...
  const [prevTurn, setPrevTurn] = useState<"team-1" | "team-2">("team-2");
  const [currTurn, setCurrTurn] = useState<"team-1" | "team-2">("team-1");
  const [winner, setWinner] = useState<"team-1" | "team-2" | null>(null);
  const [gameOver, setGameOver] = useState<boolean>(false);

  // Check for winner
  const checkWinner = (currentGrid: Grid): "team-1" | "team-2" | null => {
  ...
  // Check for draw
  const checkDraw = (currentGrid: Grid): boolean => {
  ...
  const handleBoxClick = (index: number) => {
    ...
    if (gameOver || grid[index] || 
        ...
        (currTurn === "team-2" && prevTurn !== "team-1")) return;
    ...
    socket.emit("box-clicked", { boxIndex: index, team: currTurn } as GameEvent);
  ...
  useEffect(() => {
    ...
    socket.on("box-clicked", (data: GameEvent) => {
      const updatedGrid = [...grid];
...
console.log(currTurn,prevTurn)
    ...
    socket.on("game-over", (data: WinnerData) => {
  ...
  return (
    <div className="game-section container mx-auto p-4">
      <div className="border rounded-xl p-3 flex flex-col justify-center items-center space-y-10 py-10">
        {/* Previous player display code remains the same */}
        
        {/* Tic-Tac-Toe Grid */}
        <div className="tic-tac-toe-grid grid grid-cols-3 gap-2 border p-4 rounded">
          {grid.map((mark, index) => (
            <div
              key={index}
              className={`h-20 w-20 border flex items-center justify-center text-xl font-bold cursor-pointer hover:bg-gray-100 
                ${mark === "X" ? "text-blue-500" : "text-red-500"}`}
        ...
        <div>
          ...
          >
            {currTurn === "team-1" ? "Team 1's Turn" : "Team 2's Turn"}
          </button>
        </div>
        <div className="border py-5 px-2 rounded-full">
          <span className="border p-3 rounded-full">30</span>
        </div>
      </div>
    </div>
  );
